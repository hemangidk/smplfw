...
public class CustReportListener implements IReporter {
	...
	private String reportTitle = "Doeex Test Results";
	private String reportFileName = "DoeexSmoke-report" + CustUtil.getCurrentDateTimeStamp() + ".html";
...
//					writer.println("<h1>" + testContext.getName() + "</h1>");
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				for (ITestResult testResult : tests.getResults(method)) {
					...
					if (hasThrowable) {
						String str = Utils.shortStackTrace(exception, true).toString();
	...
	protected void generateExceptionReport(Throwable exception, ITestNGMethod method) {
		...
		writer.print(Utils.shortStackTrace(exception, true).toString());
		writer.println("</div>");
	}
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) throws IOException {
		...
		m_testIndex = 1;
		titleRow("ENV URL: "+ PropertyUtil.getProperty(TestBed.getEnvFilePath(), "URL"), 10);
		for (ISuite suite : suites) {
			if (suites.size() >= 1) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				String browser = PropertyUtil.getProperty(TestBed.getEnvFilePath(), "browser");
